...
page_header(
    "Birikma ma'lumoti",
    "Formula yoki nom bo'yicha qidiring. Tanlangan birikmalarning xossalari va xavflari keltirilgan; "
    "boshqa hamma narsa oddiy qoidalar asosida nomlanadi.",
    eyebrow="Ma'lumotnoma",
)

st.session_state.setdefault(QUERY_KEY, "H2SO4")
term = st.text_input("Formula yoki nom", key=QUERY_KEY, placeholder="H2SO4, sulfuric acid, baking soda")

if not term.strip():
    st.info("Formula yoki nom kiriting.")
    st.stop()
...
if compound:
    ...
    with left:
        ...
        if compound.common_name:
            st.markdown("**Boshqa nomlari**")
            st.caption(compound.common_name)
        if compound.melting_point:
            st.markdown("**Erish harorati**")
            st.caption(compound.melting_point)
    with right:
        if compound.uses:
            st.markdown("**Nima uchun ishlatiladi**")
            st.caption(compound.uses)
        if compound.hazards:
            st.markdown("**Foydalanish va xavflar**")
            st.warning(compound.hazards, icon="⚠")
else:
    st.caption(
        "Ma'lumotnoma jadvalida yo'q. Yuqoridagi nom standart nomlash qoidalaridan "
        "olingan va molyar massa formuladan hisoblangan."
    )

if parsed:
    st.divider()
    st.markdown("#### Nimadan tashkil topgan")
    ...
    if ions and len(parsed.composition) > 2:
        ...
        for ion in ions:
            ...
            st.caption(f"{ion} — {name}, zaryadi {charge:+d}. Uni bir butun sifatida tenglashtiring.")

if len(matches) > 1:
    st.divider()
    st.markdown("#### Boshqa mos keluvchilar")
...
with st.expander(f"Barcha {len(COMPOUNDS)} ta ma'lumotnoma birikmalarini ko'rish"):
